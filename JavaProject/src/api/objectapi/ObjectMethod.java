...
public class ObjectMethod {
  ...
  public static void main(String[] args) {
    ...
    // Obejct.toString() : 객체의 문자정보 리턴(하위 클래스에서 재정의해서 개발자의도의 출력을 진행하도록 함)
    ...
    // Object.finalize() : 객체 소멸자, GC 객체를 소멸시키기 직전 finalize()를 호출
    // 기본적으로 실행내용 없음, 개발자가 객체가 소멸되기 전에 실행할 코드가 있다면 Object.finalize() 재정의

    // System.gc() : Garbage Collector 호출, 바로 호출 되지는 않고 메모리 부족하거나 CPU가 한가할 때 JVM에의해
    // 실행
    // finalize() : 언제 호출될지 모름
